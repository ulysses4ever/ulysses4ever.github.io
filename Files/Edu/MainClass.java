...
//		n = n.multiply(four);
		...
		System.out.println("estimatedTime 1: " + estimatedTime);
		
		startTime = System.currentTimeMillis();
		fib2(n);
		estimatedTime = System.currentTimeMillis() - startTime;
		System.out.println("estimatedTime 2: " + estimatedTime);



	}
	
	public static void fib1(BigInteger n) {
		BigInteger f1 = BigInteger.ONE;
		BigInteger f2 = f1; 
		BigInteger f3 = f2.add(f1);
		final BigInteger two = f3;
		BigInteger sum = BigInteger.ZERO;
		while (f3.compareTo(n) <= 0) {
			if (f3.mod(two).equals(BigInteger.ZERO))
				sum = sum.add(f3);
			f1 = f2;
			f2 = f3;
			f3 = f1.add(f2);
	...
	public static void fib2(BigInteger n) {
		final BigInteger four = new BigInteger("4");
		BigInteger f1 = new BigInteger("2");
		BigInteger f2 = new BigInteger("8"); 
		BigInteger f3 = new BigInteger("34");
		BigInteger sum = f1.add(f2);
		while (f3.compareTo(n) <= 0) {
			sum = sum.add(f3);
			f1 = f2;
			f2 = f3;
			f3 = f2.multiply(four).add(f1);
		}
		System.out.println("sum: " + sum);		
	}
