...
-- (AnyType means either a UniqueType or Type is accepted; we work under the
-- conservative assumption that any AnyType is going to be unique.)
...
-- a protocol
listenRes : proc -> proc -> Actions -> Bool -> AnyType
...
-- Commands
data Cmd : proc -> List proc -> List proc -> AnyType -> AnyType where
...
data CIO : proc -> List proc -> List proc -> AnyType -> AnyType where
     Return : {a : AnyType} -> a -> CIO me xs xs a
     Action : {a : AnyType} -> Cmd me xs xs' a -> CIO me xs xs' a
     Seq    : {a : AnyType} -> Cmd me xs xs' a -> 
...
chanPart `(Protocol.End) = Just [TextPart "End of channel"]
